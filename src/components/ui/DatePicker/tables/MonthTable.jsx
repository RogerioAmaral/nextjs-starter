...
const MonthTable = (props) => {
    ...
    const months = range.map((month, index) => {
        ...
        return (
            <button
                key={month}
                className={classNames('month-picker-cell', active && !disabled
                    ? 'month-picker-cell-active bg-primary'
                    : 'text-gray-800 dark:text-gray-100', !active && !disabled && 'hover:bg-gray-100', disabled && 'month-picker-cell-disabled')}
                disabled={disabled}
                type="button"
