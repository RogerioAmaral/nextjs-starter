    ...
    const years = range.map((year) => {
        ...
        return (
            <button
                key={year}
                disabled={disabled}
                className={classNames('year-picker-cell', active && !disabled
                    ? 'year-picker-cell-active bg-primary'
                    : 'text-gray-800 dark:text-gray-100', !active && !disabled && 'hover:bg-gray-100', disabled && 'year-picker-cell-disabled')}
                type="button"
